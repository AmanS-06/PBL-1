...
def _exhaustive(total_budget, priority_order, vby_cat, lam=2.0, cat_caps=None):
    ...
    # --- THE FIX: Cap candidates to Top 10 per category to prevent a Cartesian explosion ---
    optimized_pools = []
    for c in cats:
        # Sort vendors in this category by score and take ONLY the top 10
        cands = sorted(vby_cat.get(c, []), key=lambda v: v.norm_score, reverse=True)
        optimized_pools.append(cands[:10])

    # Now iterate over the capped pools (max 100,000 combinations instead of millions)
    for combo in itertools.product(*optimized_pools):
        ...
        if cat_caps and any(combo[i].price > cat_caps.get(cats[i], 1e18)
            ...
            continue
        obj = sum(w.get(cats[i], 0) * combo[i].norm_score for i in range(len(cats)))
        if obj > best_obj:
            best_obj = obj
...
def recommend_auto(total_budget, priority_order, vby_cat, lam=0.8):
    ...
    # --- THE FIX: Premium Tier Auto-Upgrade Loop ---
    # Instead of just hunting for a strictly higher score, we allow the AI to upgrade to
    # Premium Luxury vendors if they maintain an elite score (within 0.08 of the current best)
    # --- THE FIX: Premium Tier Auto-Upgrade Loop (Ping-Pong Prevented) ---
    ...
    while changed and savings > 0:
        ...
        for cat in priority_order:
            if cat not in sel: continue
            cur = sel[cat]
            cur_p = cur.price if cur else 0.0
            cur_score = cur.norm_score if cur else -1.0
            
            ups = []
            for v in vby_cat.get(cat, []):
                # 1. MUST be strictly more expensive (Prevents the infinite ping-pong loop)
                # 2. MUST be affordable with our current savings pool
                if v.price <= cur_p or v.price > cur_p + savings:
                    continue 
                
                # 3. MUST be an actual upgrade (either better score, or premium luxury within 0.08)
                if v.norm_score >= cur_score - 0.08:
                    ups.append(v)
                    
            if ups:
                # Prioritize strictly better scores first, then prioritize utilizing budget
                best = max(ups, key=lambda v: (v.norm_score > cur_score, v.price))
                ...
                sel[cat] = best
                changed = True
                break # Restart the loop to cascade remaining savings
    ...
    return {
        ...
        "obj":        round(obj, 4),
        "solver":     solver + " + strict premium upgrades",
    }
